...
2. **年度过完才生成年度报告**：调度器每年 1 月的 `day_of_month` 日触发，
   生成**上一年全年**（如 2027-01-01 生成 2026 年年度报告）；
   季度模式下也会自动附加这个年度任务，即一年过完同样会出年度报告。
3. **start_date 起始日期**：调度器启动时，先自动补跑
   “从 `start_date` 到今天所有已完整结束的季度”，然后进入周期等待。
   上一年度报告如果还不存在，也会一并补跑（如 `start_date=2025-06-30` 时
   会补 2025 年年度报告 + 2025Q3/Q4、2026Q1/Q2 等所有已结束季度）。
4. **缺数据自动预处理**：调度器生成某期报告前，会先检查该期的
   图库/统计值是否就绪（图库有图、统计值有“位置统计”）；缺失时自动调用
   数据处理管道（秒级→日级→图库/统计值→季度/年度总结）再生成报告。
   前提是 `preprocess/config.json` 里的 `raw_data_dir`、`daily_dir` 配置正确。
...
>
> 手动生成年度报告只填年份：
> `python run_agent.py --bridge mishuihe --mode yearly --year 2025`
> 当前年份（尚未过完）会明确报错，不会误生成今年的年度报告。
...
- **生成报告**：选模式（**季度** / 年度 / 月 / 周 / 手动）→ 季度模式选
  **季度号 1~4 + 年份**、年度模式选**年份**，不需要填日期 → 开始生成。
