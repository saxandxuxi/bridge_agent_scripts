...
class ReportReviewer:
    ...
    def review_report(self, source_text: str, report_text: str,
        ...
        if not report_text.strip():
            ...
            return result
        system = (
            "你是桥梁健康监测报告审查专家。审查“生成的报告”，只做以下两件事，"
            "其他一律不查、不报（宁缺毋滥）：\n"
            "只输出 JSON：{\"issues\": [], \"repairs\": []}，除上述两类外没有任何问题。\n"
            "1) 总结/结论段落润色：只处理 3.x 小结 与 4.监测结论与建议 段落。"
            ...
            "   - 数值/位置与季度统计一致 -> 不报。\n"
            "2) 删除冗余图注：无编号、紧挨正规编号图注（图N.N-N）、常居中的重复"
            "图注文字，输出 caption 修复，hint 必须写“删除”。编号正规图注"
            "（图N.N-N 开头）不动；原报告图纸标题（xx测点布置图/示意图/平面图/"
            "立面图/断面图/结构图/流程图等）保留。\n"
            "上下文：\n"
            ...
            "repairs.type 只允许 caption（hint=删除）与 summary。"
        )
